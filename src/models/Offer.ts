...
export interface IOffer extends Document {
   ...
   * Document.save() method, which caused TypeScript error ts(2430).
   * Stores the discount label shown below the price (e.g. "Save 20%").
   * The frontend maps this field as: offer.discount → "Save 20%"
   */
  discount?: string;
...
const offerFeatureSchema = new Schema<IOfferFeature>(
  {
    text: {
      type: String,
      required: true,
